...
class Buffer(object):
    ...
    def update(self, layer, feat):
        ...
        if self._pipeline is False:
            ...
            self._f_buf[layer] = self.__feat_concat(layer, feat) #陈旧特征
            # 通信，与计算并行
    ...
    def __gloo_all_to_all(self, send_gpu, send_cpu, recv_cpu, recv_gpu, tag, corr, avg=None, forward=True):
        #发送send_gpu中的数据到send_cpu(cpu中转）到recv_cpu到recv_gpu
        rank, size = dist.get_rank(), dist.get_world_size()
    ...
    def __feat_transfer(self, epoch, layer, feat):
        ...
        if self._backend == 'gloo':
            ...
            self._f_cuda_event[layer].record(self._comm_stream) #告诉__gloo_all_to_all和update函数中的wait:特征传输完毕
            if self._corr_feat:
